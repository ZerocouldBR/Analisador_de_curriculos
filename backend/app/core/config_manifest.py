...
CONFIG_MANIFEST: list[dict[str, Any]] = [
    ...
    # ================================================================
    {
        ...
        "fields": [
            ...
            _field("vector_search_pre_filter_threshold", "Threshold Pre-Filtro", "number",
                   "Threshold minimo antes de aplicar filtros na busca vetorial",
                   min_value=0.0, max_value=1.0, step=0.05),
            _field("confidence_coverage_divisor", "Divisor Cobertura", "number",
                   "Minimo de chunks para 100% de cobertura (normalizacao)",
                   min_value=1, max_value=20, step=1),
            _field("search_result_highlight_chars", "Chars Destaque", "number",
                   "Maximo de caracteres no trecho destacado do resultado",
    ...
    # ================================================================
    {
        ...
        "fields": [
            ...
            _field("ocr_max_images_per_docx", "Max Imagens DOCX", "number",
                   ...
                   min_value=1, max_value=50, step=1),
            _field("ocr_psm_modes", "Modos PSM", "list_int",
                   "Modos PSM do Tesseract para tentativa adaptativa (ex: 3,6,4)",
                   placeholder="3,6,4"),
        ],
    },
    ...
    # ================================================================
    {
        ...
        "fields": [
            ...
            _field("chunk_min_size", "Tamanho Minimo", "number",
                   ...
                   min_value=10, max_value=1000, step=10),
            _field("chunk_max_content_size", "Tamanho Max Conteudo", "number",
                   "Tamanho maximo do conteudo por chunk armazenado no banco",
    ...
    # ================================================================
    {
        ...
        "fields": [
            _field("max_upload_size_mb", "Tamanho Max Upload (MB)", "number",
                   ...
                   min_value=1, max_value=100, step=1),
            _field("supported_upload_extensions", "Extensoes Permitidas", "list_str",
                   "Extensoes de arquivo aceitas para upload (ex: .pdf,.docx,.doc,.txt)",
                   placeholder=".pdf,.docx,.doc,.txt,.rtf,.odt,.jpg,.jpeg,.png"),
    ...
    {
        "category": "sourcing",
        "label": "Sourcing Hibrido",
        "icon": "Hub",
        "description": "Configuracoes do modulo de sourcing de candidatos de multiplas fontes",
        "fields": [
            _field("sourcing_enabled", "Habilitar Sourcing", "boolean",
                   "Ativar modulo de sourcing hibrido de candidatos",
                   restart_required=True),
            _field("sourcing_sync_interval_days", "Intervalo de Sync (dias)", "number",
                   "Intervalo padrao entre sincronizacoes automaticas",
                   min_value=1, max_value=90, step=1),
            _field("sourcing_max_sync_candidates", "Max Candidatos por Sync", "number",
                   "Maximo de candidatos processados por sincronizacao",
                   min_value=10, max_value=5000, step=10),
            _field("sourcing_snapshot_retention_days", "Retencao de Snapshots (dias)", "number",
                   "Tempo de retencao de snapshots historicos em dias",
                   min_value=30, max_value=3650, step=30),
            _field("sourcing_external_request_timeout", "Timeout Requisicoes (s)", "number",
                   "Timeout para requisicoes a APIs externas de sourcing",
                   min_value=5, max_value=120, step=5),
            _field("sourcing_dedup_threshold", "Threshold Deduplicacao", "number",
                   "Score minimo para considerar candidato duplicata (0.0-1.0)",
                   min_value=0.0, max_value=1.0, step=0.05),
            _field("sourcing_dedup_email_weight", "Peso Email (Dedup)", "number",
                   "Peso do email na deduplicacao de candidatos",
                   min_value=0.0, max_value=1.0, step=0.05),
            _field("sourcing_dedup_phone_weight", "Peso Telefone (Dedup)", "number",
                   "Peso do telefone na deduplicacao de candidatos",
                   min_value=0.0, max_value=1.0, step=0.05),
            _field("sourcing_dedup_name_weight", "Peso Nome (Dedup)", "number",
                   "Peso do nome na deduplicacao de candidatos",
                   min_value=0.0, max_value=1.0, step=0.05),
            _field("sourcing_dedup_linkedin_weight", "Peso LinkedIn (Dedup)", "number",
                   "Peso do URL do LinkedIn na deduplicacao",
                   min_value=0.0, max_value=1.0, step=0.05),
            _field("sourcing_dedup_name_fuzzy_threshold", "Threshold Fuzzy Nome", "number",
                   "Threshold de similaridade fuzzy para comparacao de nomes (0.0-1.0)",
                   min_value=0.0, max_value=1.0, step=0.05),
            _field("sourcing_merge_priority_order", "Prioridade de Merge", "list_str",
                   "Ordem de prioridade para resolucao de conflitos entre fontes",
                   placeholder="linkedin,manual,csv_import,xlsx_import,webhook,external_partner"),
        ],
    },

    # ================================================================
    # Tuning Avancado de LLM
    # ================================================================
    {
        "category": "llm_tuning",
        "label": "Tuning Avancado LLM",
        "icon": "Psychology",
        "description": "Parametros avancados de retry, ranking e limites do LLM",
        "fields": [
            _field("llm_token_decay", "Decay de Tokens", "number",
                   "Reducao de tokens por tentativa de retry",
                   min_value=100, max_value=2000, step=100),
            _field("llm_min_tokens", "Min Tokens Resposta", "number",
                   "Minimo de tokens por resposta do LLM",
                   min_value=100, max_value=4000, step=100),
            _field("llm_character_limits", "Limites de Caracteres", "list_int",
                   "Limites de caracteres por tentativa de retry (lista decrescente)",
                   placeholder="32000,24000,16000,10000,6000"),
            _field("llm_chunks_per_retry", "Chunks por Retry", "list_int",
                   "Numero de chunks por tentativa de retry (lista decrescente)",
                   placeholder="15,12,8,5,3"),
            _field("llm_character_limit_fallback", "Limite Chars Fallback", "number",
                   "Fallback de limite de caracteres quando fora do range de retries",
                   min_value=1000, max_value=50000, step=1000),
            _field("llm_chunks_per_retry_fallback", "Chunks Retry Fallback", "number",
                   "Numero de chunks por retry em caso de fallback",
                   min_value=1, max_value=20, step=1),
            _field("llm_section_max_chunks", "Max Chunks por Secao", "number",
                   "Maximo de chunks por secao do curriculo no contexto",
                   min_value=1, max_value=20, step=1),
            _field("llm_semantic_weight", "Peso Semantico (Ranking)", "number",
                   "Peso do score semantico no ranking de resultados",
                   min_value=0.0, max_value=1.0, step=0.05),
            _field("llm_keyword_weight", "Peso Keywords (Ranking)", "number",
                   "Peso de keywords no ranking de resultados",
                   min_value=0.0, max_value=1.0, step=0.05),
            _field("llm_domain_score_multiplier", "Multiplicador Dominio", "number",
                   "Multiplicador aplicado ao score de dominio",
                   min_value=0.5, max_value=3.0, step=0.1),
            _field("keyword_idf_default", "IDF Padrao", "number",
                   "IDF padrao para keywords na extracao",
                   min_value=0.1, max_value=5.0, step=0.1),
            _field("keyword_idf_domain", "IDF Dominio", "number",
                   "IDF para keywords de dominio (boost para termos especializados)",
                   min_value=0.1, max_value=5.0, step=0.1),
            _field("keyword_idf_long_word_multiplier", "Multiplicador Palavra Longa", "number",
                   "Multiplicador IDF para palavras longas (mais especificas)",
                   min_value=1.0, max_value=3.0, step=0.1),
            _field("keyword_max_results", "Max Keywords Extraidas", "number",
                   "Maximo de keywords retornadas pela extracao",
                   min_value=5, max_value=100, step=5),
            _field("job_matching_keyword_repeat_threshold", "Threshold Repeticao Keyword", "number",
                   "Repeticoes minimas para bonus de keyword no matching",
                   min_value=1, max_value=10, step=1),
            _field("job_matching_direct_text_weight", "Peso Texto Direto (Matching)", "number",
                   "Peso da busca direta no texto no matching de vagas",
                   min_value=0.0, max_value=1.0, step=0.05),
            _field("job_matching_indexed_weight", "Peso Indexado (Matching)", "number",
                   "Peso da busca indexada no matching de vagas",
                   min_value=0.0, max_value=1.0, step=0.05),
        ],
    },
]
